...
/**
  ...
  *  (mouseover, mouseleave, .etc) please set [isMouseEvent] option to true. [eventArguments]
  *  allows you to set additional options for an event. For example, {bubbles: true} .
  */
...
module.exports = `function dispatchEventToElement(options) {
  ...
  try {
    ...
    if (options.eventArguments) {
      tgtEvent = (options.isMouseEvent) ? new MouseEvent(options.eventType, options.eventArguments) :
        new Event(options.eventType, options.eventArguments);
    } else {
      tgtEvent = (options.isMouseEvent) ? new MouseEvent(options.eventType) :
